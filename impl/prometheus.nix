  ...
  config = {
    services = {
      ...
      prometheus = {
        ...
        port = 58608; # ./string_to_port.py prometheus
        globalConfig = {
          scrape_interval = "3m";
        };
        exporters = {
          node = {
            enable = true;
            port = 50073; # ./string_to_port.py prometheus.exporters.node
            listenAddress = "127.0.0.1";
          };
          smartctl = {
            enable = true;
            port = 44232; # ./string_to_port.py prometheus.exporters.smartctl
            listenAddress = "127.0.0.1";
          };
          systemd = {
            enable = true;
            port = 31412; # ./string_to_port.py prometheus.exporters.systemd
            listenAddress = "127.0.0.1";
          };
          blackbox = {
            port = 30779; # ./string_to_port.py prometheus.exporters.blackbox
            listenAddress = "127.0.0.1";
        ...
        ++ (lib.trace.warn "Clean this up" (builtins.map (proto: {
          ...
          relabel_configs = [
            ...
            }
            {
              ...
              replacement = "127.0.0.1:${builtins.toString config.prometheus.exporters.blackbox.port}";
            }
          ];
        }) [ "tcp" "udp" "icmp" ]))
        ;
        ...
        exporters = {
          ...
          journald-exporter = {
            ...
            units = lib.mkOption {
              ...
              default = lib.trace.warn "Move these to service-level config, caddy, sshd, etc." {
                "caddy.service" = {
                  filters_regex = [
